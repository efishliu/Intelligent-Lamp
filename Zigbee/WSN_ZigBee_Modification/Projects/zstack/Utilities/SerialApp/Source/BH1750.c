...
void Delay_ms_1750(uint Time);//n ms延时
void light(void);
...
/****************************
        延时函数
*****************************/
void Delay_us_1750(uint timeout) //1 us延时uint16 timeout )
{
  while (timeout--)
  {
    asm("NOP");
...
uchar RcvByte()
{
  ...
  for(BitCnt=0;BitCnt<8;BitCnt++)
      {       
        SCL=0;      
        Delay_us_1750(5); //时钟低电平周期大于4.7us
